...
from pymunk import Space, Poly, Body,  PivotJoint , PinJoint, ShapeFilter, RotaryLimitJoint, SimpleMotor
...
class Catapult:
    ...
    def __init__(self, x: int, y: int, r : int, space: Space) -> None:
        self.space = space
        self.is_making_shot = False
        self.collision_group = ShapeFilter(group = 1)
        self.frame_body = Body()
        self.frame_body.position = x,y
        self.frame_shape = Poly(self.frame_body,((0,r),(-r,-r),(r,-r)))
        ...
        self.arm_limit = RotaryLimitJoint(self.arm.body,self.frame_body, 0, radians(225))
        ...
        self.counterweight, self.cw_joint = self.get_counterweight((x + arm_length / 4,y + 0.4 * r), (x + arm_length / 4, y + r), space)


        hook_coord = x - arm_length * 0.75, y + r
        self.hook = Ball((hook_coord), 5, space)
        self.hook.shape = ShapeFilter
        ...
        # self.ammo = self.get_ammo(self.rect_vertices[1], 1, 50, space)
    ...
    def get_wheel(self, coords:Tuple[int,int], space : Space) -> Ball:
        ...
        wheel.body.mass = 10000
        wheel.shape.filter = self.collision_group
        space.add(PivotJoint(self.frame_body, wheel.body, self.frame_body.world_to_local(coords), (0,0)))
        return wheel
    
    def get_arm(self, attachment: Tuple[int,int], space: Space, arm_length :int)->Rectangle:
        
        arm = Rectangle(attachment[0] - arm_length/4,attachment[1],arm_length, 5, space)
    ...
    def get_counterweight(self,coords: Tuple[int,int], attachment: Tuple[int,int], space: Space)-> Tuple[Ball, PinJoint]:
        cw = Ball(coords, 20, space)
        cw.body.mass = 30000
    ...
    def detach_ammo(self):
        if not self.ammo_joint:
            return
        if degrees(self.arm.body.angle-self.frame_body.angle) > -60:
            return
    ...
    def move_arm(self):
        ...
        if degrees(self.arm_limit.max) < -55:
            return
        self.arm_limit.min -= 0.05
        self.arm_limit.max -= 0.05

    def reload(self):
        self.is_making_shot = False
        ammo_coord = (self.hook.body.position.x, self.hook.body.position.y - 20)
        self.ammo = Ball(ammo_coord, 5, self.space)
