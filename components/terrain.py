...
class Terrain:
    def __init__(self, display_width: int, min_y: int, max_y: int, step: int, space: Space) -> None:
        ...
        self.left_blocks: List[TerrainBlock] = []
    ...
    def update(self, shift_x: float)-> None:
        rx = self.blocks[-1].body.position.x #rightmost block position
        lx = self.blocks[0].body.position.x #leftmost block position
